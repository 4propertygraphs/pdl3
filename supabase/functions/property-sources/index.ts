...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    // Fetch WordPress/4PM data
    if (fourpmKey) {
      try {
        const response = await fetch(
          `https://api.stefanmars.nl/api/properties`,
          {
            headers: {
              token: apiToken,
              key: fourpmKey,
        ...
        if (response.ok) {
          ...
          // Find property with matching ListReff
          const property = Array.isArray(data)
            ? data.find((p: any) => p.ListReff === listReff)
            : null;
    ...
    if (daftKey) {
      try {
        const response = await fetch(
          `https://api.stefanmars.nl/api/daft?key=${daftKey}&id=${listReff}`,
          {
            headers: {
              token: apiToken,
            },
    ...
    if (myhomeKey) {
      try {
        const response = await fetch(
          `https://api.stefanmars.nl/api/myhome?key=${myhomeKey}&id=${listReff}`,
          {
            headers: {
              token: apiToken,
            },
