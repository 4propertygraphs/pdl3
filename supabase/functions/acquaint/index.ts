...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (!apiKey || !propertyId) {
      return new Response(JSON.stringify({ error: 'API key and property ID are required' }), {
        status: 400,
    ...
    if (!response.ok) {
      return new Response(JSON.stringify({ error: 'Failed to fetch from Acquaint API', status: response.status }), {
    ...
    return new Response(JSON.stringify(matchingProperty), {
      status: 200,
      headers: { ...corsHeaders, 'Content-Type': 'application/json' },
    });
  } catch (error) {
